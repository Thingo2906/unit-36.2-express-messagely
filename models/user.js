...
class User {
  ...
  static async get(username) { 
    const result = await db.query(`SELECT username, first_name, last_name, 
                                  phone, join_at, last_login_at FROM users WHERE 
                                  username = $1`, [username]);
    //console.log(result) ;
   // debugger  ;                        
    // everytime have where we need to check the result                              
    if (!result.rows[0]) {
  ...
  static async messagesFrom(username) {
    ...
    // we need to map an array to get info for each message
  ...
  static async messagesTo(username) { 
    const result = await db.query(`SELECT m.id, m.from_username, m.body, m.send_at, m.read_at,
                     u.first_name, u.last_name, u.phone FROM messages AS m JOIN users AS u
